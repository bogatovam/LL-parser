...
Представленных выше алгоритм не использует дополнительной памяти, но при этом будет не оптимальным по времени по следующим причинам. Во первых, для выполнения шага 2 необходимо объединение множеств, а эта операция не из "дешевых". Во вторых, на шаге 3 необходимо сравнение множеств, что тоже требует прохода по ним. И на шаге 4 необходим проход по всем множествам.

Я предполагаю, что наивная реализация алгоритма выше будет не оптимальной.
Один из способов оптимизировать работу алгоритма - использование дополнительной памяти. В моей реализации это 2 дополнительных массива:
		char** waiting = calloc(2, sizeof(char*));
		bool* ready = calloc(g->countNterm, sizeof(bool));

Двумерный массив `waiting` хранит пары нетерминалов: <Нетерминал1><Нетерминал2>, где Нетерминал1 ждет готовности Нетерминала2. То есть в множество FIRST(<Нетерминал1>)=...<Нетерминал2>. Это поможет избежать многократного прохода по "уже собранным множествам". Также я ввожу двумерный массив флагов `ready`, каждый i-й элемент которого сообщает, собрано ли множество нетерминала i.
