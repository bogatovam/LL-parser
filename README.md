...
Для каждого нетерминала необходимо хранить все его продукции. Для того, чтобы не нужно было парсить большую строку с продукциями при необходимости доступа к ним, я предлагаю в качестве записи использовать структуру со следующими полями
...
При первом проходе по продукциям нетерминалов формируется массив `waiting` и в множества FOLLOW заносятся необходимые множества FIRST. Далее мы проходим по массиву "ожидающих", пока в течении итерации будет хотя бы одно изменение какого то множества. При обработке пары ожидающих они не удаляются, а маркируются.
